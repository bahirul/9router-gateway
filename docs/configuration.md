...
- Identity: disabled by default, display model name `9Router`, creator/owner `9router-gateway`.
...
- Model identity override toggle, display name, and creator/owner name.
...
When model identity override is enabled, the gateway prepends a short system-level instruction to OpenAI Chat Completions, OpenAI Responses, and Anthropic Messages requests so the assistant identifies as the configured display name for model/name/LLM questions and the configured creator/owner for created-by, made-by, built-by, owner, developer, provider, powered-by, hosted-by, or behind-this questions. This is prompt-based behavior, not response rewriting, so adversarial or conflicting prompts may still affect the answer.
